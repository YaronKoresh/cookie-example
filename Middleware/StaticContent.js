...
export const StaticContent = async function(request,response,next){
	...
	try {
		...
		if( request.originalUrl.includes("/.") || request.originalUrl.includes("./") || request.originalUrl.includes("..") || request.originalUrl.includes("\\") ){
			// Detect some common bad requests, used by nasty hackers!
			console.log(`Bad request from ${request.ip} (${request.method} ${request.originalUrl})`);
			return response.status(403).end();
		}

		if( request.method.toLowerCase() != "get" || request.originalUrl.includes("/api/") ){
			// Simply, do not check for static content permissions, if the visitor do not want any static content at all...
			console.log(`API request (${request.method} ${request.originalUrl}): no role checks needed`);
			return next();
		}

		const isAdmin = request.session && request.session.role && request.session.role == "Admin";
		const isUser = request.session && request.session.role && request.session.role == "User";
