...
ExampleRouter.get("/", async function(request, response){
	...
	try {
		...
		if( request.session && request.session.role && request.session.role == "Admin" ){

			response.status(200).send("Admin");

		} else if (request.session && request.session.role && request.session.role == "User") {
	...
	try {
		if( request.session && request.session.role && request.session.role == "Admin" ){
			...
			if( body.password == adminPassword ){
				...
				request.session.but_i_want_to_create_a_second_junk_just_for_fun = "Hello again!!";
				response.status(200).end();
				return;
			} else {
